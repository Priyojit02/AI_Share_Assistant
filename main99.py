...
async def sync_hub(hub_name: str, force: bool = False):
    """
    Sync hub - either with SharePoint or re-process uploaded files
    """
    try:
        # Load metadata
        metadata = vector_store_manager.load_metadata(hub_name)
        if not metadata:
            raise HTTPException(
                status_code=404,
                detail=f"Hub '{hub_name}' not found"
            )

        # Check if it's a SharePoint linked hub
        if metadata.get("sharepoint_link"):
        ...
        elif metadata.get("source") == "upload":
            # File upload hub sync - update sync timestamp
            logger.info(f"Syncing uploaded files for hub: {hub_name}")

            # Load existing manifest
            manifest = vector_store_manager.load_manifest(hub_name)
            if not manifest or not manifest.get("files"):
                raise HTTPException(
                    status_code=400,
                    detail=f"No files found for hub '{hub_name}'"
                )

            files = manifest["files"]

            # For file upload hubs, sync updates the timestamp
            # (since we don't store original files for re-processing)
            metadata["last_synced"] = datetime.utcnow().isoformat()
            vector_store_manager.save_metadata(hub_name, metadata)
            ...
            return SyncResponse(
                ...
                message=f"Successfully synced {len(files)} uploaded files"
            )

        else:
            raise HTTPException(
                status_code=400,
                detail=f"Hub '{hub_name}' does not support sync operations"
            )
