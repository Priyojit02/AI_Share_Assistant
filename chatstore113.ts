...
interface ChatState {
  ...
  removeMessagesFromIndex: (chatId: string, startIndex: number) => void;
...
export const useChatStore = create<ChatState>()(
  persist(
    (set, get) => ({
      ...
      stopStreaming: () => {
        const { streamingCleanup, loadingChatId, chats, addMessage } = get();
        if (streamingCleanup) {
          streamingCleanup();
        }
        
        // Update the last assistant message to indicate stopped, or add if none
        if (loadingChatId) {
          const chat = chats.find(c => c.id === loadingChatId);
          if (chat && chat.messages.length > 0) {
            const lastMsg = chat.messages[chat.messages.length - 1];
            if (lastMsg.role === 'assistant') {
              // Update existing assistant message
              const updatedMessages = [...chat.messages];
              let stoppedContent = '❌ Request stopped by user.';
              // For normal mode, if the content ends with '...', remove it and keep the generated text
              if (lastMsg.content.endsWith('...')) {
                const generatedText = lastMsg.content.slice(0, -3);
                if (generatedText.trim()) {
                  stoppedContent = generatedText.trim();
                }
              }
              ...
              const updatedChats = chats.map(c => c.id === loadingChatId ? { ...c, messages: updatedMessages } : c);
              set({ chats: updatedChats });
            } else {
              // Add new stopped message
              addMessage(loadingChatId, { 
                role: 'assistant', 
      ...
      startJobPolling: (chatId: string, jobId: string) => {
        ...
        set({ loadingChatId: chatId, isLoading: true, currentJob: { status: 'running', job_id: jobId, chat_id: chatId } });
          ...
          try {
            ...
            if (status.status === 'completed') {
              ...
              const updatedChats = chats.map(c => {
                if (c.id === chatId) {
                  const lastMessage = c.messages[c.messages.length - 1];
                  const errorMessage = `❌ Error: ${status.error || 'Job failed'}`;
                  if (lastMessage && lastMessage.role === 'assistant') {
                    ...
                    return { ...c, messages: [...c.messages, { role: 'assistant' as const, content: errorMessage, timestamp: new Date().toISOString() }] };
                  }
                }
                return c;
              });
      ...
      restartJobStreaming: (chatId: string, jobId: string) => {
        ...
        // Set streaming state
        set({ loadingChatId: chatId, isLoading: false, isStreaming: true, currentJob: { status: 'running', job_id: jobId, chat_id: chatId } });
      ...
      removeMessagesFromIndex: (chatId, startIndex) => {
        const { chats } = get();
        const updatedChats = chats.map(chat => {
          if (chat.id === chatId) {
            const messages = chat.messages.slice(0, startIndex);
      ...
      sendMessage: async (content) => {
        ...
        // Re-evaluate agent mode after potential chat creation
        ...
        // Add empty assistant message first to show streaming cursor (only for normal mode)
        if (!isAgentMode) {
          addMessage(chatId, { role: 'assistant', content: '🤖 Agent is running...', timestamp: new Date().toISOString() });
        } else {
          // For agent mode, add a processing message
          addMessage(chatId, { role: 'assistant', content: '🤖 Agent is running...', timestamp: new Date().toISOString() });
        }

        // Set loading to false but keep streaming true for cursor (normal mode) or for status (agent mode)
        ...
        try {
          ...
          if (isAgentMode) {
            ...
            // Simulate streaming by revealing text in blazing fast chunks
            // Stream at maximum speed regardless of tab visibility using requestAnimationFrame
            let currentContent = '';
            let charIndex = 0;
            const chars = fullMessage.split('');
            let animationFrameId: number | null = null;
            const CHARS_PER_FRAME = 10; // Stream 10 characters per animation frame for blazing speed

            const streamNextChunk = () => {
              if (charIndex < chars.length) {
                // Stream multiple characters per frame for maximum speed
                const endIndex = Math.min(charIndex + CHARS_PER_FRAME, chars.length);
                currentContent += chars.slice(charIndex, endIndex).join('');
                updateLastMessage(chatId, currentContent + '...');
                charIndex = endIndex;

                // Use requestAnimationFrame for smooth, high-performance streaming
                animationFrameId = requestAnimationFrame(streamNextChunk);
              } else {
                updateLastMessage(chatId, fullMessage);
          ...
          const poll = async () => {
            try {
              ...
              if (status.status === 'completed') {
                ...
                const errorMsg = `❌ Error: ${status.error || 'Job failed'}`;
                const { chats } = get();
                const chat = chats.find(c => c.id === chatId);
                const lastMsg = chat?.messages[chat.messages.length - 1];
                if (lastMsg?.role === 'assistant') {
                ...
                if (attempts < maxAttempts) {
                  ...
                  const errorMsg = '❌ Error: Request timed out';
                  const { chats } = get();
                  const chat = chats.find(c => c.id === chatId);
                  const lastMsg = chat?.messages[chat.messages.length - 1];
                  if (lastMsg?.role === 'assistant') {
